...
def saveUBPMJson(allRecords):
    try:
        latest_entry = None
        latest_dt = datetime.datetime.min
        
        # 1. Saari records mein se sabse nayi (latest) dhoondna
        for userIdx, userRecs in enumerate(allRecords):
            for rec in userRecs:
                try:
                    # Date convert karna comparison ke liye
                    rec_dt = datetime.datetime.strptime(rec["datetime"], "%Y-%m-%d %H:%M:%S")
                    if rec_dt > latest_dt:
                        latest_dt = rec_dt
                        # Sirf zaroori data save karna
                        latest_entry = {
                            "user": f"User{userIdx + 1}",
                            "date": rec_dt.strftime("%d.%m.%Y"),
                            "time": rec_dt.strftime("%H:%M:%S"),
                            "sys": int(rec['sys']),
                            "dia": int(rec['dia']),
                            "bpm": int(rec['bpm']),
                            "datetime_full": rec["datetime"]
                        }
                except:
                    continue

        # 2. Agar koi reading mili hai, to file ko overwrite karna
        if latest_entry:
            f = pathlib.Path("ubpm.json")
            # Hum poora purana data ura kar sirf ye 1 object daal rahe hain
            f.write_text(json.dumps(latest_entry, indent=4, sort_keys=True))
            logger.info(f"JSON updated: Latest reading from {latest_entry['user']} saved.")
            
    except Exception as e:
        logger.error(f"Error in saveUBPMJson: {e}")
...
async def main():
    ...
    try:
        ...
        await bleClient.connect(timeout=5)  # 5 seconds timeout – fast fail if device not ready
    ...
    # omblepy.py ki line 407 ke kareeb (finally block)
    finally:
        logger.info("disconnecting...") # 'unpair' word hata diya
        if bleClient.is_connected:
            # await bleClient.unpair()  <-- IS LINE KO DELETE YA COMMENT KAREIN
            try:
                await bleClient.disconnect()
                logger.info("Disconnected but kept paired in Windows logs.")
            except Exception as e:
...
# Purani line 'asyncio.run(main())' ko hata kar ye niche wala code likhein:
if __name__ == "__main__":
    try:
        asyncio.run(main())
    except bleak.exc.BleakDeviceNotFoundError:
        # Jab device na mile to crashing error ke bajaye ye message aaye
        print("DEVICE_NOT_FOUND: Omron not found or BT button not pressed.")
        sys.exit(0) # Program ko izzat se band karo taake loop dobara chal sake
    except Exception as e:
